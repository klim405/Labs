...
public abstract class PersonAbstract implements Person {
    ...
    public void say(Person person, String text) {
        System.out.println(name + " сообщил(-ла, -ли) " + person + " что " + text);
    }

    @Override
    public void agreeWith(Person person, Thing thing) {
        if (isPersonAgree(thing)) {
            System.out.println(name + " согласился(-лась, -лись)");
        } else {
            System.out.println(name + " отказался(-лась, -лись)");
        }
    }
